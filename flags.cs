...
namespace iFlag
{
    public partial class mainForm : Form
    {
        ...
        private bool matchRacingFlags(long flagID)
        {
            if (flagID < 0) return false;
                 ...
                 if (Convert.ToBoolean(flagID & irsdk_crossed)
                  || Convert.ToBoolean(flagID & irsdk_disqualify)) return flag("Disqualified", CROSSED_FLAG, new byte[] { COLOR_BLACK, COLOR_RED, COLOR_RED, COLOR_RED, COLOR_RED }, FAST);
            else if (Convert.ToBoolean(flagID & irsdk_yellow)) return clearedFlag("Careful And No Overtaking!", WAVING_FLAG, new byte[] { COLOR_BLACK, COLOR_YELLOW }, FAST);
            ...
            else if (Convert.ToBoolean(flagID & irsdk_black)) return flag("Serve Stop And Go Penalty", INVERTED_FLAG, new byte[] { COLOR_BLACK, COLOR_RED }, SLOW);
            ...
            else if (Convert.ToBoolean(flagID & irsdk_green)) return flag("Green", WAVING_FLAG, new byte[] { COLOR_BLACK, COLOR_GREEN }, FAST);
            ...
            else if (Convert.ToBoolean(flagID & irsdk_repair)) return flag("Too Much Damage", MEATBALL_FLAG, new byte[] { COLOR_BLACK,COLOR_YELLOW, COLOR_RED }, SLOW);
            ...
            else if (Convert.ToBoolean(flagID & irsdk_white)) return flag("Last Lap", FLASHING_FLAG, new byte[] { COLOR_DIM_WHITE, COLOR_BLACK }, SLOW);
            ...
            // else if (Convert.ToBoolean(flagID & irsdk_startHidden)) return flag("None", SIMPLE_FLAG, new byte[] { COLOR_BLACK, COLOR_BLACK }, SLOW);
                  ...
                  && eventType == "Race") return flag("One Lap To Green", ONE_FLAG, new byte[] { COLOR_BLACK, COLOR_GREEN }, SLOW);
            else return false;
        }
        ...
        private bool matchStartingFlags(long flagID)
        {
            ...
            else if (Convert.ToBoolean(flagID & irsdk_startGo)) return flag("Startlights: Go!", WAVING_FLAG, new byte[] { COLOR_GREEN, COLOR_GREEN }, FAST);
            else return false;
        }
        ...
        private bool matchRepairsFlags(long flagID)
        {
            if (!this.repairsOverlayModuleMenuItem.Checked) return false;
                 ...
                 if (flagID == REPAIRS_MANDATORY) return flag("Service: Mandatory Repairs", FLASH_CIRCLE_FLAG, new byte[] { COLOR_BLACK, COLOR_RED }, SLOW);
            else if (flagID == REPAIRS_OPTIONAL) return flag("Service: Optional Repairs", FLASH_CIRCLE_FLAG, new byte[] { COLOR_BLACK, COLOR_YELLOW }, SLOW);
            else if (flagID == REPAIRS_DONE) return flag("Service: Repairs Done", FLASH_CIRCLE_FLAG, new byte[] { COLOR_BLACK, COLOR_GREEN }, SLOW);
            else return false;
        }
